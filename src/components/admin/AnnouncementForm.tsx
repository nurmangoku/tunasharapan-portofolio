...
const ANNOUNCEMENT_IMAGES_BUCKET = 'announcement_images';
...
const AnnouncementForm: React.FC<AnnouncementFormProps> = ({ onSubmit, initialData, isSubmittingOverall }) => {
  ...
  const deleteOldImageFromStorage = async (imageUrl: string | undefined | null) => {
    ...
    try {
      ...
      if (pathParts.length > 1) {
        ...
        await supabase.storage.from(ANNOUNCEMENT_IMAGES_BUCKET).remove([imagePath]);
      }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (imageFile) {
      ...
      try {
        ...
        await supabase.storage.from(ANNOUNCEMENT_IMAGES_BUCKET).upload(filePath, imageFile);
        const { data: publicUrlData } = supabase.storage.from(ANNOUNCEMENT_IMAGES_BUCKET).getPublicUrl(filePath);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 sm:p-8 rounded-xl shadow-lg">
      ...
      <div>
        <label htmlFor="imageFile-input" className="block text-sm font-medium text-gray-700 mb-1">Gambar Pengumuman (Opsional)</label>
        <div className="mt-1 flex flex-col items-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
          {(imagePreview || currentImageUrl) ? (
            <div className="relative group w-full max-w-xs mx-auto">
              <img src={imagePreview || currentImageUrl || undefined} alt="Preview Pengumuman" className="h-40 w-full object-contain rounded-md mx-auto"/>
      ...
      <div>
        <label htmlFor="excerpt" className="block text-sm font-medium text-gray-700 mb-1">Cuplikan (Opsional)</label>
        <textarea id="excerpt" value={excerpt} onChange={(e) => setExcerpt(e.target.value)} rows={3} 
                  placeholder="Ringkasan singkat (akan digenerate jika kosong)"
                  className="mt-1 block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-colors"/>
      </div>
      <div>
